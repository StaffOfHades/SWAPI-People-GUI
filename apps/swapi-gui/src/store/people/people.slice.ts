...
export const { decreasePage, increasePage, resetPeople, setPage, setSearchTerm } = peopleActions;
...
} = peopleAdapter.getSelectors<RootState>((state) => state[PeopleFeatureKey]);

export const selectMaxPage = createSelector(
  (state: RootState) => state[PeopleFeatureKey].count,
  (state: RootState) => state[PeopleFeatureKey].perPage,
  (count, perPage) => Math.ceil(count / perPage)
);

export const selectPeoplePage = createSelector(
  selectPeople,
  (state: RootState) => state[PeopleFeatureKey].page,
  (state: RootState) => state[PeopleFeatureKey].perPage,
  (people, page, perPage) => {
